...
steps =10000#500
intervals = 10000#500
timestep_short = 0.0005/4.0
timestepdevide_medium = 1#5
timestepdevide_short = 4
...
gamma = 2.0#0.1#1.0#0.0#0.01#1.0
...
for pidCG in range(num_particlesCG):
    for trot in range(nTrotter):
        allParticlesAT.append([pidCG*nTrotter + trot + 1, # add here these particles just temporarily
                            Real3D(x[pidCG]+ 0.0*(rand.random()-0.5), y[pidCG]+ 0.0*(rand.random()-0.5), z[pidCG]+ 0.0*(rand.random()-0.5)), # position
