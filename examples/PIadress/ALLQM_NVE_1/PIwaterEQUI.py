...
gamma = 0.0#2.0#0.1#1.0#0.0#0.01#1.0
...
adress = espressopp.integrator.AdressPI(system, vl, ftpl, nTrotter)
...
print 'Medium timestep = ', integrator.dt * timestepdevide_short
...
print "step, time (ps), T, Eb, EAng, EPI, ELj, Ek, Etotal, DeltaE"
...
outfile.write(fmt%(0, 0, T, Eb, EAng, EPI, ELj, Ek, Etotal, 0.0))
print (fmt%(0, 0, T, Eb, EAng, EPI, ELj, Ek, Etotal, 0.0))
...
for s in range(1, intervals + 1):
    ...
    EPI = integrator.computeRingEnergy()
    Etotal = Ek+Eb+EAng+ELj+EPI
    Esum += np.abs((Etotal-Eref)/Eref)
    DeltaE = (1.0/s) * Esum
    outfile.write(fmt%(step, time, T, Eb, EAng, EPI, ELj, Ek, Etotal, DeltaE))
    print (fmt%(step, time, T, Eb, EAng, EPI, ELj, Ek, Etotal, DeltaE))
    #dump_conf_gro.dump()
    #dump_conf_gro_adr.dump()
