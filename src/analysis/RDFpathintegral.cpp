...
namespace espressopp {
  namespace analysis {
    ...
    python::list RDFpathintegral::computeArray(int rdfN) const {
                ...
                if (it2 != fixedtupleList->end()) {  // Are there atomistic particles for given CG particle? If yes, use those for calculation.
                      ...
                      for (std::vector<Particle*>::iterator it3 = atList.begin();
                          ...
                          if ( (at.position()[0] < Xmax) && (at.position()[0] > Xmin) ){
                            int id = at.id();
                            //conf[id] = at.position();
                            data tmp = { at.position(), at.type(), vp.id(), at.pib() };
                            conf[id] = tmp;
                          }
                      }
                }
              ...
              if ( (molP1 != molP2) && (pibP1 == pibP2) && (coordP2[0] < Xmax) && (coordP2[0] > Xmin) ){
                  ...
                  if( ( (typeP1 == target1) && (typeP2 == target2) ) || ( (typeP1 == target2) && (typeP2 == target1) ) ){
                      ...
                      if( bin < rdfN){
                        ...
                        // Normalization (Raffaello's paper)
    ...
    real RDFpathintegral::compute() const {
      return -1.0;
    }

    void RDFpathintegral::registerPython() {
      using namespace espressopp::python;
      class_<RDFpathintegral, bases< Observable > >
        ("analysis_RDFpathintegral", init< shared_ptr< System >, int, int, real >())
        .def("compute", &RDFpathintegral::computeArray)
      ;
    }
  }
}
